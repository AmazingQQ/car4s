        ...
        <div class="card-toolbar clearfix">
          <form id="f1" class="row" method="post" action="list" role="form">
			<div class="form-group col-md-6">
              ...
              <input type="text" class="form-control" id="name" name="name" value="${param.name }" placeholder="请输入配件名称">
            </div>
			<div class="form-group col-md-6">
            ...
            <div class="form-group col-md-12">
              ...
              <button type="button" class="btn btn-primary" onclick="location.href='${ctx}/fittings/save'"><i class="mdi mdi-plus"></i> 新增</button>
        ...
        <div class="card-body">
          ...
          <form action="del" method="post" id="tableForm">
            <table class="table table-bordered">
              ...
              <tbody>
              	<c:forEach items="${list }" var="fitting">
	                <tr>
	                  <td>
	                    <label class="lyear-checkbox checkbox-primary">
	                      <input type="checkbox" name="ids" value="${fitting.id }"><span></span>
	                    </label>
	                  </td>
	                  <td>${fitting.id }</td>
	                  <td>${fitting.name }</td>
	                  <td>${fitting.brand }</td>
	                  <td><my:dict data="${dictMap['CAR_UNIT'] }" value="${fitting.unit }"/></td>
	                  <td>${fitting.price }</td>
	                  <td>${fitting.type }</td>
	                  <td>${fitting.remark }</td>
	                  <td>
	                    <div class="btn-group">
	                      <a class="btn btn-xs btn-default" href="${ctx }/fittings/edit?id=${fitting.id}" title="编辑" data-toggle="tooltip"><i class="mdi mdi-pencil"></i></a>
	                      <a class="btn btn-xs btn-default" href="javascript:;" onclick="deleteUser(${fitting.id})" title="删除" data-toggle="tooltip"><i class="mdi mdi-window-close"></i></a>
	                    </div>
	                  </td>
	                </tr>
                </c:forEach>
